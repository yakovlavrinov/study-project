...
function slider({container, slide, nextArrow, prevArrow, totalCounter, currentCounter, wrapper, field}) {
  ...
  next.addEventListener('click', () => {
      if (offset == deleteNotDigital(width) * (slides.length - 1)){
          offset = 0;
      } else {
          offset += deleteNotDigital(width);
      }
  ...
  prev.addEventListener('click', () => {
      if (offset == 0){
          offset = deleteNotDigital(width) * (slides.length - 1);
      } else {
          offset -= deleteNotDigital(width);
      }
  ...
  dots.forEach(dot => {
      dot.addEventListener('click', (e) => {
          ...
          slideIndex = slideTo;
          offset = deleteNotDigital(width) * (slideTo - 1);
